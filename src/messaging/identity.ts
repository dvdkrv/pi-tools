...
export const QUIET_GUIDANCE = 'In an explicitly joined group, use peer_message only for a requested exchange or a substantive blocker, contract change, or completion needed by your assigned work. Every send declares a kind: a notice forbids all reverse replies, a request permits exactly one referenced reply, and a reply must cite that request and cannot be acknowledged. Never disguise a response as a new notice or request. Queue the message and continue that work; do not wait for a reply, poll status, send periodic updates, or acknowledge receipts unless the protocol and human assignment require it. If blocked on a reply, report the blocker and end the current run so queued messages can arrive. Do not automatically retry uncertain messaging outcomes.';
